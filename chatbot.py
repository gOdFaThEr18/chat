...
# creating dictionary that maps each line and ids
id2line = {}
for line in lines:
    _line = line.split(' +++$+++ ')
    if len(_line) == 5:
...
for line in lines:
    _line2 = line.split(' +++$+++ ')
    if len(_line2) == 5 and i<=200:
        id2line2[_line2[0]] = _line2[4]
        i+=1
...
# create a dictionary that maps each word with its no. of occurences
word2count={}
...
for answer in clean_answers:
    for word in answer.split():
        if word not in word2count:
            ...
            word2count[word]+=1
# create dictionary that map words from questions and answers to unique integer
threshold=20
...
for word,count in word2count.items():
    if count>=threshold:
        ...
        word_number+=1
# addding the last tokens in these two dictionaries
tokens=['<PAD>','<EOS>','<OUT>','<SOS>']
for token in tokens:
    questionswords2int[token]=len(questionswords2int)+1
for token in tokens:
    answerswords2int[token]=len(answerswords2int)+1    
# create inverse dictionary of the answers dictionary
